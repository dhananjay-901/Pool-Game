...
BALL_RADIUS = 10
POCKET_RADIUS = 18
FRICTION = 0.99  # LOWER friction = balls roll longer (was 0.985)
MIN_VELOCITY = 0.02  # Lower threshold = balls can move slower before stopping
BORDER_SIZE = 20
MASS = 1.0
...
class Ball:
    ...
    def move(self):
        ...
        # Wall Collisions - Keep balls inside the green area
        ...
        if self.pos.x < inner_left:
            ...
            self.vel.x *= -0.9  # Slight energy loss on wall bounce
...
def resolve_collision(b1, b2):
    """Enhanced elastic collision with proper momentum transfer"""
    if not b1.active or not b2.active: return
    
    dist = b1.pos.distance_to(b2.pos)
    if dist < b1.radius + b2.radius:
        # 1. Calculate collision normal
        collision_normal = b2.pos - b1.pos
        if collision_normal.length() == 0:
            collision_normal = pygame.math.Vector2(1, 0)
        collision_normal = collision_normal.normalize()
        
        # 2. Separate overlapping balls
        overlap = (b1.radius + b2.radius - dist) / 2
        b1.pos -= collision_normal * overlap
        b2.pos += collision_normal * overlap
        
        # 3. Calculate relative velocity
        rel_vel = b1.vel - b2.vel
        
        # 4. Calculate velocity along collision normal
        vel_along_normal = rel_vel.dot(collision_normal)
        
        # 5. Do not resolve if velocities are separating
        if vel_along_normal > 0:
            return
        
        # 6. Calculate impulse scalar (elastic collision)
        restitution = 0.95  # Bounciness (0-1, higher = more bounce)
        impulse = -(1 + restitution) * vel_along_normal
        impulse /= (1/b1.mass + 1/b2.mass)
        
        # 7. Apply impulse to both balls
        impulse_vector = collision_normal * impulse
        b1.vel += impulse_vector / b1.mass
        b2.vel -= impulse_vector / b2.mass
...
def check_pockets(balls):
    ...
    for ball in balls:
        ...
        for px, py in pockets:
            ball_pos = pygame.math.Vector2(px, py)
            if ball.pos.distance_to(ball_pos) < POCKET_RADIUS:
                ball.active = False
...
def ai_shoot(cue_ball, object_balls):
    ...
    # Find best target (closest to pocket)
    best_target = None
    best_dist = float('inf')
    ...
    for target in targets:
        for pocket in pockets:
            dist = target.pos.distance_to(pygame.math.Vector2(pocket))
            if dist < best_dist:
                best_dist = dist
                best_target = target
    
    target = best_target if best_target else random.choice(targets)
    diff = target.pos - cue_ball.pos
    angle = math.atan2(diff.y, diff.x)
    
    # Add slight error for realism
    error = random.uniform(-0.08, 0.08)
    power = random.uniform(10, 16)  # Slightly higher power
...
def main():
    ...
    while running:
        ...
        if game_state == "MOVING":
            ...
            # Check collisions multiple times for accuracy
            for _ in range(4):
            ...
            if not check_balls_moving(balls):
                # Keep turn if someone potted a ball
